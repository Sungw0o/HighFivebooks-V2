...
export function CheckoutPage() {
  ...
  const shippingFee = policy === null ? 0 : itemsTotal >= policy.minOrderAmount ? 0 : policy.standardShippingFee
  ...
  const submit = async () => {
    ...
    if (memberId === null && orderPassword.trim().length < 4) {
      return setFormError('비회원 주문 비밀번호를 4자 이상 입력해주세요.')
    }
    ...
    try {
      ...
      if (tossClientKey) {
        ...
        await requestTossPayment(tossClientKey, methodName, {
          amount: order.totalAmount,
          orderId: order.orderKey,
          orderName: order.orderName,
          customerName: receiverName.trim() || undefined,
          successUrl: `${origin}/payment/success?method=${encodeURIComponent(methodName)}`,
      ...
      } else if (err?.message === 'TOSS_SDK_LOAD_FAILED') {
        setFormError('결제 모듈을 불러오지 못했습니다. 잠시 후 다시 시도해주세요.')
      } else {
    ...
    <div className="min-h-screen bg-ink">
      ...
      <main className="px-4 pb-24 sm:px-10">
        ...
        {loading ? (
          ...
          <EmptyState message="주문할 상품이 없습니다." actionLabel="책 보러가기" onAction={() => navigate('/books')} />
        ) : (
                ...
                <div className="mt-5 grid gap-4 sm:grid-cols-2">
                  ...
                  <div className="flex gap-2 sm:col-span-2">
                    ...
                    <button
                      ...
                      onClick={() => {
                        void openDaumPostcode((result) => setAddress(result.roadAddress))
                      }}
                    >
                      주소 검색
                    </button>
